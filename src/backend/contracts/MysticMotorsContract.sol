...
contract MysticMotors is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    //NOTE Token values incremented for gas efficiency
    uint256 public maxSalePlusOne = 501;

    uint256 public tokenPricePresale = 0.02 ether;
    uint256 public tokenPricePublic = 0.02 ether;

    // amount of nfts to be minted per address plus one
    uint256 public saleAllowancePlusOne = 21;
    ...
    constructor() ERC721A("Mystic Motors", "MM") {
      //merkleRoot = 0x;

      baseURI = "https://bafybeiahekcekq7zqnft2wcbknbk6ysfms6dj57uskiltgbva4cfidemcq.ipfs.nftstorage.link/";
    }
    ...
    {
        require(quantity + mintsPerAddress[msg.sender] < saleAllowancePlusOne, "Exceeds allowance!");
        uint256 currentSupply = totalSupply();

        _safeMint(msg.sender, quantity);
        mintsPerAddress[msg.sender] += quantity;

        return (currentSupply, totalSupply());
    }
    ...
    {
        require(quantity + mintsPerAddress[msg.sender] < saleAllowancePlusOne, "Exceeds allowance!");
        require(verify(merkleRoot, leaf, proof), "Not Whitelisted!");
        uint256 currentSupply = totalSupply();
